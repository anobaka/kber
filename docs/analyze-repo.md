...
```
拉取与降噪 → AST 结构化解析 → 代码块级 LLM 归纳 → 向量化 → 模块摘要生成 → 仓库概览生成 → 存储与更新
```
...
| 层级 | 粒度 | 存储标识 | 说明 |
|------|------|----------|------|
| 代码块 | 函数/类/文件 | `block_type=function/method/class/...` | 精确定位具体实现 |
| 模块摘要 | 目录级 | `block_type=module_summary` | 回答"某模块做什么""有哪些功能" |
| 仓库概览 | 仓库级 | `block_type=repo_summary` | 回答架构/API 清单/全貌问题 |
...
- 代码块并发度：`LLM_CONCURRENCY_BLOCK`（默认 5），模块摘要并发度：`LLM_CONCURRENCY_MODULE`（默认 3）
- 速率限制：遵守阿里云百炼 API 的 QPS 限制
- 失败处理：单个代码块失败后标记 `code_block.status=failed`，下次自动重试
- 进度通知：使用飞书 Interactive Card 原地更新（而非发送新消息），显示百分比和预计剩余时间，每个阶段完成时更新为 ✅ 状态
- 超长内容处理：当 LLM 生成的知识描述超过 4500 字符时，调用 LLM 在语义边界处智能分段，每段独立 Embedding 存入 Milvus。若 LLM 分段失败则 fallback 为硬分段。Milvus 服务层保留截断安全网（varchar 字段自动截断到 schema 限制）

### 5.4 模块摘要生成

代码块知识生成完成后，对有变更的模块（目录）重新生成摘要：

1. 确定受影响的模块目录（有代码块变更或文件被删除的目录）
2. 收集该目录下所有 `status=success` 的代码块描述
3. 调用 LLM 生成模块级中文摘要（包含模块职责、主要功能、对外接口、依赖关系、中文关键词）
4. 删除 Milvus 中旧的模块摘要，插入新摘要（`block_type=module_summary`）

### 5.5 仓库概览生成

任何模块摘要变更后，重新生成仓库级全局概览：

1. 收集所有模块摘要
2. 调用 LLM 生成仓库概览（包含项目简介、架构设计、核心模块、模块关系、API 清单、技术栈、中文关键词）
3. 删除旧概览，插入新概览（`block_type=repo_summary`）
